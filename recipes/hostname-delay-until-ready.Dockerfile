...
ARG COMMANDS="elapsed-time-under export-elapsed-time exec expect-http-get-response expect-http-get-response-not sleep while-no-error subcmd spawn kill-pid wait-pid export export-dns-query exit signal enviro echo"
...
ENTRYPOINT \
    ...
    && while-no-error \
      [ \
        ...
        && subcmd [\
          export DEPENDENT_SERVICE_IP 0.0.0.0 \
          export-dns-query ${DEPENDENT_SERVICE} DEPENDENT_SERVICE_IP \
          ; echo [Found service in hosts as ${DEPENDENT_SERVICE_IP}] \
          && expect-http-get-response-not ${DEPENDENT_SERVICE_IP} 9000 / 200 \
          && export-elapsed-time DURATION \
          && echo [Waited ${DURATION} seconds for ${DEPENDENT_SERVICE_IP}:9000 to come alive] \
        ] \
      ] \
      [sleep 2] \
    ; expect-http-get-response ${DEPENDENT_SERVICE_IP} 9000 / 200 \
        && subcmd [ \
...
#  $ docker build -t presh/hostname-delay-until-ready -f recipes/hostname-delay-until-ready.Dockerfile .
#  $ docker build -t presh/env-config-file -f recipes/env-config-file.Dockerfile .
#  $ docker network create delay-net
#  $ docker run --rm -d -p 3000:3000 --network delay-net presh/hostname-delay-until-ready
