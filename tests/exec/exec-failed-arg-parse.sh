...
# desc: exec with argument parsing errors
# requires: +exec

# exec pulls in arguments outside the main loop, so we
#   need to ensure it does error checking just like the main loop.
# See arg-size-overflow.sh for where this logic comes from.
...
"${FS}" -c "exec ${long_arg}" > out.txt 2>err.txt
res=$?
