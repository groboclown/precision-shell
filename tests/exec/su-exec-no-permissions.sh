...
# desc: su-exec when run without the right permissions
# requires: +su-exec
...
if [ "$( id -u )" != "0" ] ; then
    if [ "$( id -u )" != "${UID1}" ] ; then
        "${FS}" -c "su-exec [${UID1}] [${GID1}] [${id_exec} -u]" > out.txt 2>err.txt
        res=$?
    else
        "${FS}" -c "su-exec [${UID2}] [${GID2}] [${id_exec} -u]" > out.txt 2>err.txt
        res=$?
    fi
else
    su -s "${FS}" "${user2}" -c "su-exec [${UID1}] [${GID1}] [${id_exec} -u]" > out.txt 2>err.txt
    res=$?
fi
