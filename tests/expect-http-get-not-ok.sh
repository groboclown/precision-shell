...
# desc: expect that a running local server generates an http code that isn't like an expected one.
...
"${FS}" -c "expect-http-get-response-not localhost 29446 / 210" > out.txt 2>err.txt
res=$?
kill -15 "${server_pid}"
